...
class Protocol:
    ...
    def generate_ff_params(self, component_smiles: dict):
        ...
        for mol_name, smiles in component_smiles.items():
            ...
            param_files = [
                f'{self.params_dir}/{mol_name}.itp',
                f'{self.params_dir}/{mol_name}.atp',
                f'{self.params_dir}/{mol_name}.gro',
                f'{self.params_dir}/{mol_name}.json',
                f'{self.params_dir}/{mol_name}_nb_params.json'
            ]
            
            if all(os.path.exists(f) for f in param_files):
                logger.info(f'parameters for {mol_name} already exist, skipping generation')
                # Load existing parameters
                with open(f'{self.params_dir}/{mol_name}.json', 'r') as f:
                    params = json.load(f)
                with open(f'{self.params_dir}/{mol_name}_nb_params.json', 'r') as f:
                    nb_params = json.load(f)
                    metadata = nb_params.get('metadata', {})
    ...
    def build_system(self, total_atoms: int, components_ratio: dict, working_dir: str, build_gas: bool = False):
        ...
        for component_name, molar_ratio in components_ratio.items():
            print(component_name, molar_ratio)
